...
class ForensicReport:
    ...
    def _verdict(self, attacks, findings):
        if self.metadata.get('greenlit'):
            return "GREENLIGHTED — NO ACROFORM SIGNATURES"
    ...
    def to_markdown(self):
        ...
        # ============================================================
        # GREEN: GREENLIGHTED — no AcroForm attack surface
        # ============================================================
        if self.metadata.get('greenlit'):
            reason = self.metadata.get('greenlit_reason', '')
            lines.append("## ✅ Greenlighted")
            lines.append("")
            if 'DocuSign' in reason:
                lines.append(
                    "This document is a DocuSign envelope with trusted certificate authority "
                    "infrastructure. DocuSign signatures use CA-issued certificates, "
                    "RFC 3161 timestamps, and containers validated by Adobe's Approved "
                    "Trust List (AATL). The five attack classes this engine checks for "
                    "were tested by the Bochum researchers against documents with weaker "
                    "infrastructure — self-signed certificates, missing timestamps, "
                    "deprecated padding. DocuSign's signing pipeline does not produce "
                    "these vulnerabilities.")
            else:
                lines.append(
                    "This document contains no AcroForm-based digital signature fields. "
                    "The engine searched for `/FT /Sig` form fields, `/Type /Sig` signature "
                    "dictionaries, ByteRange arrays, and PKCS#7 containers — none were found.")
                lines.append("")
                lines.append(GREENLIGHT_SCOPE)
                lines.append("")
                lines.append(GREENLIGHT_VISUAL)
                lines.append("")
                lines.append(
                    "If this document was signed via a platform like ApproveMe, HelloSign, "
                    "PandaDoc, or similar e-signature services that use visual image stamps "
                    "rather than AcroForm certificate infrastructure, the signature is outside "
                    "the scope of this engine and the document is free from the attack surface "
                    "that the Bochum papers describe.")
            lines.append("")
            ...
            lines.append("*Generated by [PDF Shadow Attack Forensic Engine v2]"
                         "(https://github.com/mortalezz/pdf-shadow-engine) — "
    ...
    def to_pdf(self, output_path):
        ...
        if self.metadata.get('greenlit'):
            ...
            story.append(Paragraph("Greenlighted", S['h2']))
            if 'DocuSign' in reason:
                story.append(Paragraph(
                    "This document is a DocuSign envelope with trusted certificate authority "
                    "infrastructure. DocuSign signatures use CA-issued certificates, "
                    ...
                    "infrastructure — self-signed certificates, missing timestamps, "
                    "deprecated padding. DocuSign's signing pipeline does not produce "
                    "these vulnerabilities.", S['body']))
            else:
                story.append(Paragraph(
                    "This document contains no AcroForm-based digital signature fields. "
                    "The engine searched for <font face='Courier'>/FT /Sig</font> form "
                    "fields, <font face='Courier'>/Type /Sig</font> signature dictionaries, "
                    "ByteRange arrays, and PKCS#7 containers — none were found.", S['body']))
                story.append(Paragraph(GREENLIGHT_SCOPE, S['body']))
                story.append(Paragraph(GREENLIGHT_VISUAL, S['body']))
                story.append(Paragraph(
                    "If this document was signed via a platform like ApproveMe, HelloSign, "
                    "PandaDoc, or similar e-signature services that use visual image stamps "
                    "rather than AcroForm certificate infrastructure, the signature is "
                    "outside the scope of this engine and the document is free from the "
                    "attack surface that the Bochum papers describe.", S['body']))

            # Footer and return early
...
''')
        ...
        if self.metadata.get('greenlit'):
            reason = self.metadata.get('greenlit_reason', '')
            h.append('<section>')
            h.append('<h2 style="border-color: var(--forest);">✅ Greenlighted</h2>')
            if 'DocuSign' in reason:
                h.append(
                    '<p>This document is a DocuSign envelope with trusted certificate authority '
                    'infrastructure. DocuSign signatures use CA-issued certificates, '
                    'RFC 3161 timestamps, and containers validated by Adobe\'s Approved '
                    'Trust List (AATL). The five attack classes this engine checks for '
                    'were tested by the Bochum researchers against documents with weaker '
                ...
                h.append(
                    '<p>This document contains no AcroForm-based digital signature fields. '
                    'The engine searched for <code>/FT /Sig</code> form fields, '
                    '<code>/Type /Sig</code> signature dictionaries, ByteRange arrays, and '
                    'PKCS#7 containers — none were found.</p>')
                h.append(f'<p>{esc(GREENLIGHT_SCOPE)}</p>')
                h.append(f'<p>{esc(GREENLIGHT_VISUAL)}</p>')
                h.append(
                    '<p>If this document was signed via a platform like ApproveMe, HelloSign, '
                    'PandaDoc, or similar e-signature services that use visual image stamps '
                    'rather than AcroForm certificate infrastructure, the signature is outside '
                    'the scope of this engine and the document is free from the attack surface '
                    'that the Bochum papers describe.</p>')
            h.append('</section>')
...
GREENLIGHT_SCOPE = (
    "This engine checks documents that use AcroForm-based digital signatures — "
    "the PDF signing mechanism defined in the Adobe PDF specification and tested "
    "by researchers at Ruhr University Bochum in CCS 2019 and NDSS 2021. "
    "AcroForm signatures are identified by the presence of /FT /Sig fields "
    "(signature form fields), /Type /Sig dictionaries (signature value objects), "
    "ByteRange arrays (signed byte coverage), and PKCS#7 cryptographic containers. "
    "All five attack classes — USF, ISA, SWA, PKCS, and SHADOW — target "
    "specifically this infrastructure and cannot exist in documents that don't use it."
)

GREENLIGHT_VISUAL = (
    "A simple visual check can still validate any electronic signature: look for "
    "the signer's identity linked to a timestamp in the same signature block. "
    "Adobe identifies timestamps as the most critical component of digital "
    "signature compliance — they lock both the signer's identity and the "
    "document itself, providing what Adobe calls 'a lock for the lock.'"
)
...
def analyze(filepath):
    ...
    if _is_docusign_envelope(parser):
        ...
        report.metadata['greenlit'] = True
        report.metadata['greenlit_reason'] = 'DocuSign trusted CA infrastructure'
        report.global_findings.append(Finding(INFO, INFO,
            "DocuSign envelope — greenlighted",
            "This document is a DocuSign envelope with trusted certificate authority "
            "infrastructure. DocuSign signatures use CA-issued certificates, "
            "RFC 3161 timestamps, and PAdES or PKCS#7 containers validated by "
            "Adobe's Approved Trust List (AATL). The five attack classes this engine "
            "checks for (USF, ISA, SWA, PKCS, SHADOW) were tested by the Bochum "
            "researchers against documents with weaker infrastructure — self-signed "
            "certificates, missing timestamps, deprecated padding. DocuSign's "
            "signing pipeline does not produce these vulnerabilities. "
    ...
    if not sig_fields:
        report.metadata['greenlit'] = True
        report.metadata['greenlit_reason'] = 'No AcroForm digital signature infrastructure'
        report.global_findings.append(Finding(INFO, INFO,
            "No AcroForm signatures — greenlighted",
            "This document contains no AcroForm-based digital signature fields. "
            "The engine searched for /FT /Sig form fields, /Type /Sig signature "
            "dictionaries, ByteRange arrays, and PKCS#7 containers — none were found. "
            + GREENLIGHT_SCOPE + " " + GREENLIGHT_VISUAL + " "
            "If this document was signed via a platform like ApproveMe, HelloSign, "
            "PandaDoc, or similar e-signature services that use visual image stamps "
            "rather than AcroForm certificate infrastructure, the signature is outside "
            "the scope of this engine and the document is free from the attack surface "
