...
PDFs signed with AcroForm-based digital signatures — the certificate-backed signing mechanism defined in the Adobe PDF specification — carry a specific attack surface: ByteRange gaps, incremental saves, PKCS#7 padding, and appearance stream manipulation. Five classes of attacks targeting this infrastructure were demonstrated by researchers at Ruhr University Bochum in two landmark papers (CCS 2019, NDSS 2021), and most of these vulnerabilities exist in millions of documents, most of them harmless. This engine ignores that noise and instead looks for the artifacts of attacks that were actually carried out: signature appearances stripped of text and replaced with bitmaps, documents modified after signing via incremental saves that redefine objects outside the signed byte range, and the telltale combination of both on the same page where one signature was tampered with and another wasn't. Documents without AcroForm signature infrastructure — those signed by platforms like DocuSign, ApproveMe, or HelloSign — are greenlighted immediately because the attack surface the Bochum papers describe does not exist in them.
